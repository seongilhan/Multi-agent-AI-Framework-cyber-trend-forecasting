...
# List for counting mentions of each PT by the harvester (PT keywords list)
mention_list = [
    ('BLOCKCHAIN',),
    ('ACCESS CONTROL',),
    ('ENCRYPTION',),
...
# Part 1: Collect URLs of all relevant documents (for each month and each PT)
...
# Part 2: Harvest and count the number of mentions of each PT during each month
# The count is within the title, abstract, and keywords
            ...
            try:
                myElem = WebDriverWait(driver, delay).until(
                    expected_conditions.presence_of_element_located(
                        (By.CLASS_NAME, "Highlight-module__akO5D")
                    ))
            ...
            while True:
                ...
                for element in text_elements:
                    try:
                        ...
                        failed = True
                        break
                    count = sum(element_text.replace("-", " ")
                                .upper()
                                .count(
                        x.upper().replace("-", " ")) for x in mention)
                    print("count:", count)
                    totalQ += count
                if not failed:
                    break
